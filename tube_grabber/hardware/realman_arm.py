...
class RealManArm:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise HardwareError(
                "未安装 RealMan Python SDK，无法导入 Robotic_Arm.rm_robot_interface"
            ) from exc
        ...
        try:
            ...
            if dof != self.expected_dof:
                raise HardwareError(
                    f"连接到 {dof} 轴机械臂，但本项目要求 {self.expected_dof} 轴右臂"
                )
            ...
            if isinstance(exc, HardwareError):
                raise
            raise HardwareError(f"连接右臂 {self.ip}:{self.port} 异常: {exc}") from exc
