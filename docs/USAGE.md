...
- YOLO Detection 识别四颗角螺丝，白色 marker 确定 K0；
- 多帧异常值剔除、中值融合和稳定性门禁；
...
默认配置声明 `rack_1` 和 `rack_2`，但 `rack_2` 必须在现场单独标定。普通 `transfer`
仍只执行同一 `rack_id` 内搬运；固定路线跨站任务使用独立的 `mobile-transfer`。普通篮筐
不属于 rack，当前固定篮子流程不使用视觉槽位标定。
...
- 新设备首次部署时，`observation_pose.confirmed` 必须先设为 `false`。

新设备首次部署时，`config/app.yaml` 中 `motion.parameters_confirmed` 也必须先设为
`false`。完成 TCP、工作空间、架面法向对齐、接近高度和抬升高度的低速真机检查后，
才能分别确认这两个锁。仓库中的数值与确认状态来自当前实验设备，不能直接视为其他
设备的安全标定。
控制器安装角为 Y=45°，因此抓放不能把 `base Z` 当作物理竖直。视觉会输出
`approach_axis_base`，所有抓取、放置、下降和抬升均沿该架面法向进行；
`vision.plane.expected_normal_base` 仅用于检查实测法向是否符合控制器安装方向。
...
`screw.pt` 必须是 YOLO Detection 模型，内部类别为 `0: item`。模型只标注架子
四角的四颗螺丝；K0 旁边额外设置一个白色 marker，但不把 marker 标成 screw。
白色胶带应与白色底板隔开并贴在绿色架面上。运行时要求恰好四个 screw 框，取中心
构成架面；HSV 高亮低饱和连通域负责找到 marker，
再以 K0 的长边邻点定义 K1。
...
该命令只开 D435 彩色流并连续运行 `cap.pt`、`screw.pt`，不连接机械臂/夹爪，
也不运行深度或运动逻辑。画面只显示盖框和标定后的 K0～K3，不显示底层螺丝框或
中心。运行前须先执行 `calibrate-corners`。最终必须在真实观察位验证 marker 方向、
四角点抖动、盖框和 GPU 帧耗时。
...
标定视角可以和正常观察位不同。程序会把两个槽圆心转换成由 K0～K3 定义的架面归一化
坐标，正常运行时再通过当前四角投影到图像中。
...
按 K0、K1、K2、K3 顺序点击或拖动到四个目标交点，每点按 Enter 确认，最后按
`s` 保存。该命令只更新展示角点，不改变槽锚点。正式 `scan` 和 CUDA 测试流只画
这四个标定点与细四边形，底层仍由四螺丝 Detection 完成定位。

### 抓放全程录像窗口

真机 `transfer` 自动打开腕部画面，不需要额外启动相机脚本：

- 静止阶段连续运行 CUDA 推理，显示 K0～K3、槽中心和盖子框/中心/置信度；
- 机械臂运动阶段不运行 YOLO，只显示实时无标注腕部画面；
- 到位检查完成后自动恢复推理；
- 展示流只用于录像，抓放坐标仍由独占相机/模型后的稳定扫描生成，不会被实时展示更新。
...
2. 操作人员确认夹爪/TCP 物理空载后直接按 Enter（输入 `q` 取消）；
3. 程序使用旧 AprilTag 流程验证过的 `movej_p` 自动进入全局观察位，并检查到位误差；
4. 启动 D435，执行首轮多帧扫描并打印预览计划；
5. 操作人员确认结果后再次直接按 Enter（输入 `q` 取消）；
6. 在同一观察位执行第二轮多帧扫描，比较现场变化，并用第二轮最新坐标重新生成抓取计划；
7. 执行张开、抓取和 150 mm 垂直上提；
8. 携管移动到目标槽高位安全走廊，不进入较低的全局观察位；
9. 当前 `destination_recheck_while_carrying: false`，放置使用抓取前完整扫描得到的目标
   坐标，避免持管遮挡四颗架面螺丝；
10. 下降放置、释放并垂直撤离后，程序自动返回全局观察位；
11. 最终多帧复扫必须确认源槽为空、目标槽占用、其他槽未异常变化，才报告任务完成。
...
架号、行列范围、源目标关系、占用、视觉、规划、执行前现场变化和人工 Enter 授权。

当前语音尚未接入。Agent 入口仍走同架 `transfer`，跨架命令会被明确拒绝；经现场确认
的固定双架路线必须显式使用 `mobile-transfer`，语音输入也不能绕过现有安全校验
直接控制硬件。
...
- `cap.pt`、`screw.pt` 都必须只有 `{0: item}`；
- screw 每帧必须恰好检出四颗角螺丝；
- K0 白色 marker 必须唯一，不能和白色底板连成同一片区域。

### 多帧稳定性失败

- 保持机械臂和试管架静止；
- 检查相机曝光、运动模糊和 USB 带宽；
- 检查螺丝框标注质量和白色 marker 的亮度/饱和度阈值；
- 检查是否存在严重遮挡；
...
这是普通 `transfer` 的预期安全行为。以下命令不会从同架入口执行，必须改用完成配置和
验收后的 `mobile-transfer`：

跨站流程分阶段验收时，先使用 `plan-mobile-pick-home` 和
